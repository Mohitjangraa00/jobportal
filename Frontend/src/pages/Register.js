...
const Register = () => {
  ...
  // Create refs at top level (not inside map)
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      await apiFetch("/auth/send-register-otp", { email: form.email, name: form.name });
      setStep(2);
      startTimer();
  ...
  const handleVerifyAndRegister = async (e) => {
    ...
    try {
      // Verify OTP first
      await apiFetch("/auth/verify-register-otp", { email: form.email, otp: otpStr });

      // Then create account
      const { confirmPassword, ...submitData } = form;
      const res = await apiFetch("/auth/register", submitData);

      if (res?.token) localStorage.setItem("token", res.token);
      if (res?.user)  localStorage.setItem("user",  JSON.stringify(res.user));

      setStep(3);
      setTimeout(()=>navigate(form.role==="recruiter"?"/employer/dashboard":"/candidate/dashboard",{replace:true}), 2000);
    } catch(err) {
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .pst-i{display:flex;align-items:flex-start;gap:16px;}
        .pst-n{width:32px;height:32px;border-radius:50%;background:rgba(99,102,241,.2);border:1px solid rgba(99,102,241,.4);display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-size:13px;font-weight:700;color:#a5b4fc;flex-shrink:0;}
        .pst-t{font-size:14px;color:rgba(255,255,255,.55);padding-top:6px;line-height:1.5;}
        .pst-t strong{color:rgba(255,255,255,.85);font-weight:500;}
        ...
        .logo-d{width:8px;height:8px;background:#6366f1;border-radius:50%;}
        ...
        .prline{flex:1;height:2px;background:#e5e7eb;margin:0 6px 14px;transition:background .3s;}
        .prline.done{background:#22c55e;}
        ...
        .sb{display:flex;gap:4px;flex:1;}
        ...
        .ck.ok{color:#22c55e;}
        .ck-dot{width:14px;height:14px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;flex-shrink:0;background:#e5e7eb;color:#94a3b8;}
        .ck.ok .ck-dot{background:rgba(34,197,94,.2);color:#22c55e;}
      ...
      <div className="rg">
        ...
        <div className="rgp">
          ...
          <div style={{position:"relative",zIndex:1}}>
            ...
            <div className="pst">
              {[
                ["01","Create & verify your account — Secure signup with email verification."],
                ["02","Browse or post jobs — Explore curated listings or attract top talent."],
                ["03","Apply or hire — One click to apply; smart tools to shortlist candidates."],
              ].map(([n,t])=>(
                <div className="pst-i" key={n}>
                  <div className="pst-n">{n}</div>
                  <div className="pst-t">{t}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="rgs">
          <div className="rgc">
            ...
            <div className="logo"><span className="logo-d"/>JobPortal</div>
            ...
            {step < 3 && (
              <div className="pr">
                {[["1","Details"],["2","Verify Email"],["3","Done"]].map(([num,lbl],i)=>{
                  const n=i+1;
                  const st=step>n?"done":step===n?"active":"pend";
                  return (
                    <React.Fragment key={num}>
                      ...
                      {i<2&&<div className={`prline${step>n?" done":""}`}/>}
                    </React.Fragment>
                  );
                })}
              </div>
              ...
              <>
                ...
                <form onSubmit={handleFormSubmit}>
                  <div className="fg">
                    ...
                    <input type="text" name="name" value={form.name} onChange={handleChange} placeholder="John Doe" className="fi" autoComplete="name"/>
                  </div>
                  <div className="fg">
                    <label className="fl">Email Address <span>*</span></label>
                    <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="you@example.com" className="fi" autoComplete="email"/>
                  </div>
                  <div className="fg">
                    <label className="fl">Password <span>*</span></label>
                    <div className="pw">
                      <input type={showPw?"text":"password"} name="password" value={form.password} onChange={handleChange} placeholder="Min. 6 characters" className="fi" autoComplete="new-password"/>
                      <button type="button" className="shb" onClick={()=>setShowPw(!showPw)}>{showPw?"Hide":"Show"}</button>
                    </div>
                    {form.password&&(
                      <div className="sr">
                        <div className="sb">{[1,2,3,4].map(i=><div key={i} className="sbar" style={{background:i<=strength.level?strength.color:"#e5e7eb"}}/>)}</div>
                        <span className="slbl" style={{color:strength.color}}>{strength.label}</span>
                      </div>
                    )}
                  </div>
                  <div className="fg">
                    ...
                    <div className="pw">
                      <input type={showCPw?"text":"password"} name="confirmPassword" value={form.confirmPassword} onChange={handleChange} placeholder="Re-enter password" className={`fi${form.confirmPassword&&form.confirmPassword!==form.password?" err":""}`} autoComplete="new-password"/>
                      <button type="button" className="shb" onClick={()=>setShowCPw(!showCPw)}>{showCPw?"Hide":"Show"}</button>
                    </div>
                  ...
                  {checks.length>0&&(
                    <div className="cklist">
                      {checks.map(([ok,txt])=>(
                        <div key={txt} className={`ck${ok?" ok":""}`}>
                          <div className="ck-dot">{ok?"✓":"○"}</div>{txt}
                        </div>
                      ))}
                    </div>
                  )}

                  <button type="submit" className="subbtn" disabled={loading}>
                    {loading?<><div className="spin"/>Sending OTP...</>:"Continue — Verify Email →"}
                  </button>
                </form>
            ...
            {step===2&&(
              <>
                ...
                <p className="cs" style={{marginBottom:18}}>
                  We sent a 6-digit code to <span className="emhl">{form.email}</span>. Enter it below to complete signup.
                </p>
                  ...
                  <div className="orow" onPaste={handleOtpPaste}>
                    {otp.map((digit,i)=>(
                      <input key={i} ref={otpRefs[i]}
                        ...
                        className={`obox${digit?" f":""}`}
                        autoFocus={i===0}
                      />
                    ))}
                  </div>

                  <button type="submit" className="subbtn" disabled={loading||otp.join("").length<6}>
                    {loading?<><div className="spin"/>Verifying & creating account...</>:"Verify & Create Account →"}
                  </button>
                </form>

                <div className="rsrow" style={{marginTop:14}}>
                  Didn't get it?{" "}
                  <button className="rsbtn" onClick={handleResend} disabled={resendTimer>0||loading}>
